...
export default {
    ...
    syncToSummary(newVal) {
      ...
      if (newVal || asserts.isBoolean(newVal) || asserts.isNumber(newVal)) {
        ...
        if (this.isMultiSelect) {
          ...
          const includeSummary = this.injRoot.filterSummary.filter(
            (item) => item.prop === this.prop
          );
          ...
          this.injRoot.filterSummary = this.injRoot.filterSummary
            ...
            .concat(
              newVal.map((e) => ({
                prop: this.prop,
                label: "",
                value: this.formaterInRenderFactor
                  ? this.formaterInRenderFactor(e, row)
                  : e,
                optionKey: e, // 作为标识的字段
                isMultiple: true, // 是多选
              }))
        ...
        const formatValue = this.formaterInRenderFactor
          ? this.formaterInRenderFactor(newVal, row)
          : newVal;
        if (formatValue || asserts.isNumber(formatValue)) {
          // 如果存在预览数据，就更新它，不存在就push一个新的
          const summaryItem = this.injRoot.filterSummary.find(
            (item) => item.prop === this.prop
          );
          if (summaryItem) {
      ...
      // 删除值
      const summaryIndex = this.injRoot.filterSummary.findIndex(
        (item) => item.prop === this.prop
      );
      if (summaryIndex !== -1) {
  ...
  render() {
    ...
    if (asserts.isFunction(render)) {
      ...
      itemContent = RenderFactory.render(
        h,
        renderOptions,
        vmodel,
        this._props,
      ...
      const options = cloneDeep(renderOptions) || {};
